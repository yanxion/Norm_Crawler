...
class Crawler_Proxy_Util:
    def __init__(self):
        ...
        self.db = SQL_Connect()
        self.db.connect_mysql(os.path.dirname(__file__))
    ...
    def get_random_proxy(self, use_set_proxy=False):
        ...
        if use_set_proxy:
            ...
            while while_cnt <= 5:
                while_cnt += 1
                proxy_data = self.db.select_sql("SELECT method, ip, port, account, passwd FROM proxy ORDER BY RAND() LIMIT 1 ;")
                print proxy_data[1],
                if proxy_data[3] and proxy_data[4]:
                    proxies = {
                        "http": proxy_data[0] + "://" + proxy_data[3] + ':' + proxy_data[4] + '@' + proxy_data[1] + ":" + proxy_data[2],
                        "https": proxy_data[0] + "://" + proxy_data[3] + ':' + proxy_data[4] + '@' + proxy_data[1] + ":" + proxy_data[2]
                    }
                else:
                    proxies = {
