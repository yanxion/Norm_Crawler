...
class transfer_comment():
    ...
    def init_setting(self, query_datetime=''):
        ...
        index_data = self.db.select_sql(
                "select comment_index from url_list_python where query_time = '{}'".format(self.url_list_python['query_time']))
        ...
        if index_data[0] == 0:
            ...
            if select_data[0][0]:
                self.url_list_python['comment_index'] = select_data[0][0]
                self.url_list_python['comment_total'] = select_data[0][1]
            else:
                self.url_list_python['comment_index'] = 0
                self.url_list_python['comment_total'] = 0
            self.db.insert_sql(
                'UPDATE url_list_python SET comment_index = %s, comment_total = %s WHERE query_time = "%s"' %
                (self.url_list_python['comment_index'], self.url_list_python['comment_total'], self.url_list_python['query_time']))
            ...
            data = self.db.select_sql(
                    "select comment_index, comment_total, run_tag from url_list_python where query_time = '{}'".format(
                        self.url_list_python['query_time']))
            self.url_list_python['comment_index'] = data[0]
            self.url_list_python['comment_total'] = data[1]
    ...
    def main(self):
        ...
        if self.url_list_python['run_tag'] == 0:
            if self.url_list_python['comment_index'] != -1:
                # 抓 index 值
                handle_cnt = self.url_list_python['comment_index']
                # 每次從資料庫抓 query_count_num 的筆數
                for low in range(self.url_list_python['comment_index'], self.url_list_python['comment_total'],
                    ...
                    self.db.insert_sql(
                        'update url_list_python set comment_index = "%s" where query_time = "%s";' % (str(handle_cnt), self.url_list_python['query_time']))
                ...
                self.db.insert_sql('update url_list_python set comment_index = -1, comment_total = "%s",'
                                   ...
                                   (comment_total_cnt, self.time.time_to_str(self.time.now()), self.url_list_python['query_time']))
